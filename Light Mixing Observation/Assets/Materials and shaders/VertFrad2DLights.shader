// Upgrade NOTE: replaced '_Object2World' with 'unity_ObjectToWorld'
// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'

Shader "My Shaders/2D Lights/VertFrad2DLights" {
	Properties{
		_Color("Light Color", Color) = (1, 1, 1, 1)
		_MainTex("Base (RGB)", 2D) = "white" {}

		_LightPosition("Light Position", Vector) = (1, 1, 1, 1)	//This will be set in the c# script
		_LightDirection("Light Direction", Vector) = (0 , 0, 0, 0) //This will also be set inside the script

		_RightVector("Right Vector", Vector) = (0, 0, 0, 0)
		_LeftVector("Left Vector", Vector) = (0, 0, 0, 0)

		_RightFogVector("", Vector) = (0, 0, 0, 0)
		_LeftFogVector("", Vector) = (0, 0, 0, 0)

		_ActualRightVector("Right Vector", Vector) = (0, 0, 0, 0)
		_ActualLeftVector("Left Vector", Vector) = (0, 0, 0, 0)

		_ActualRightFogVector("", Vector) = (0, 0, 0, 0)
		_ActualLeftFogVector("", Vector) = (0, 0, 0, 0)

		_Distance("Distance of light", Range(0, 30)) = 0.5
		_NoFadeDistance("", Range(0, 30)) = 0.5


		_LightMixtureAdjustmentController("", Range(0, 2)) = 0.4
	}

	SubShader{
		ZWrite Off

		//Blend DstColor Zero
		//Blend SrcAlpha OneMinusSrcAlpha
		//Blend DstColor SrcColor
		//Blend One One
		//Blend DstColor Zero
		//Blend DstColor SrcColor

		//BlendOp Sub

		Tags{"Queue" = "Transparent" "IgnoreProjector" = "True" "RenderType" = "Transparent"}

		LOD 200
		/*Pass{
			CGPROGRAM
			//#pragma surface surf Lambert alpha
			#pragma vertex vert
			#pragma fragment frag
			#include "UnityCG.cginc"

			float4 _Color;
			sampler2D _MainTex;

			float4 _LightPosition;
			float4 _LightDirection;

			float4 _RightVector;
			float4 _LeftVector;

			float4 _RightFogVector;
			float4 _LeftFogVector;

			float _Distance;
			float _NoFadeDistance;


			struct vertInput{
				float4 pos : POSITION;
				float2 texCoord : TEXCOORD0;
			};

			struct vertOutput{
				float4 pos : SV_POSITION;
				float2 texCoord : TEXCOORD0;
			};

			/*struct Input{
				float2 uv_MainTex;

				float3 worldPos;	//This provides us with the world position of each pixel
			};

			vertOutput vert(vertInput i){
				vertOutput o;
				o.pos = mul(UNITY_MATRIX_MVP, i.pos);
				o.texCoord = i.texCoord;
				return o;
			}

			half4 frag(vertOutput o) : COLOR{
				half4 c;
				c.rgb = tex2D(_MainTex, o.texCoord).rgb;
				
				c.a = _Color.a;
				return _Color;
			}*/

			/*void surf(Input IN, inout SurfaceOutput o){
			
				//get the distance from the point of the light to the pixel
				float reductionAlpha = smoothstep(_Distance, _NoFadeDistance, distance(_LightPosition, IN.worldPos));

				float4 n = tex2D(_MainTex, IN.uv_MainTex);

				//Get the direction of the pixel WRT the light position
				float3 pixelRelToLight = IN.worldPos - _LightPosition.xyz;

				float d1 = dot(_RightVector, normalize(pixelRelToLight));
				float d2 = dot(_LeftVector, normalize(pixelRelToLight));

				float d3 = dot(_RightFogVector, normalize(pixelRelToLight));
				float d4 = dot(_LeftFogVector, normalize(pixelRelToLight));

			
				if(d1 > 0 && d2 > 0){
					o.Albedo =  _Color;
					o.Alpha = reductionAlpha;
				}
				else{
					o.Alpha = 0;
				}

				if(d3 > 0 && d1 < 0 ){
					float sm = smoothstep(abs(dot(_RightVector, _RightFogVector)), 0,
											 abs(dot(normalize(pixelRelToLight) , _RightFogVector)));
					o.Albedo =  _Color;
					o.Alpha = (1 - sm) * reductionAlpha;
				}

				if(d4 > 0 && d2 < 0){
					float sm = smoothstep(abs(dot(_LeftVector, _LeftFogVector)), 0,
											 abs(dot(normalize(pixelRelToLight) , _LeftFogVector)));
					o.Albedo = _Color;
					o.Alpha = (1-sm) * reductionAlpha;
				}
			}*/

			//ENDCG

		//}*/
	
	
		
		//BlendOp RevSub
		//Blend SrcAlpha OneMinusSrcAlpha
		Blend One One
		
		GrabPass{}
		
		Pass{
			
			
			CGPROGRAM
				//#pragma surface surf Lambert alpha
				#pragma vertex vert
				#pragma fragment frag
				#include "UnityCG.cginc"

				sampler2D _GrabTexture;
				float4 _Color;


				float4 _LightPosition;
				float4 _LightDirection;

				float4 _RightVector;
				float4 _LeftVector;

				float4 _RightFogVector;
				float4 _LeftFogVector;

				float4 _ActualLeftVector;
				float4 _ActualRightVector;
				float4 _ActualLeftFogVector;
				float4 _ActualRightFogVector;

				float _Distance;
				float _NoFadeDistance;

				float _LightMixtureAdjustmentController;

				struct vertexInput{
					float4 vertex : POSITION;
					float4 worldPos : TEXCOORD2;
				};

				struct vertexOutput{
					float4 vertex : POSITION;
					float4 uvGrab : TEXCOORD1;
					float4 worldPos : TEXCOORD2;
				};

				vertexOutput vert(vertexInput i){
					vertexOutput o;
					o.vertex = UnityObjectToClipPos(i.vertex);
					o.uvGrab = ComputeGrabScreenPos(o.vertex);

					o.worldPos =  mul(unity_ObjectToWorld, i.vertex);
					return o;
				}

				half4 frag(vertexOutput o) : COLOR{
					fixed4 pixelLocation = UNITY_PROJ_COORD(o.uvGrab);
					fixed4 c = tex2Dproj(_GrabTexture, UNITY_PROJ_COORD(o.uvGrab)) ;


					float reductionAlpha = smoothstep(_Distance, _NoFadeDistance, distance(_LightPosition, o.worldPos));

					//float4 n = tex2D(_MainTex, IN.uv_MainTex);

					//Get the direction of the pixel WRT the light position
					//float3 pixelRelToLight = IN.worldPos - _LightPosition.xyz;
					float3 pixelRelToLight = o.worldPos - _LightPosition.xyz;

					float d1 = sign( dot(_RightVector, normalize(pixelRelToLight)));
					float d2 = sign( dot(_LeftVector, normalize(pixelRelToLight)));

					float d3 = dot(_RightFogVector, normalize(pixelRelToLight));
					float d4 = dot(_LeftFogVector, normalize(pixelRelToLight));

					float4 finalColor;
			
					if(d1 > 0 && d2 > 0){
						finalColor = ( (c + _Color) + _LightMixtureAdjustmentController) * reductionAlpha;
						//o.Albedo =  _Color;
						//o.Alpha = reductionAlpha;
					}
					else{
						finalColor = half4(0, 0, 0, 0);
					}

					if( d1 < 0 && d3 > 0){
						/*float sm = smoothstep(0, abs(dot(_RightVector, _RightFogVector)),
												 abs(dot(normalize(pixelRelToLight) , _RightFogVector)));

						finalColor = half4(( c + (_Color)  + _LightMixtureAdjustmentController) * reductionAlpha * sm );*/

						float sm = smoothstep(abs (dot(_ActualRightVector, _ActualRightFogVector)), 1, 
													abs(dot(normalize(pixelRelToLight) , _ActualRightFogVector)));
						
						finalColor = half4(( c + (_Color)  + _LightMixtureAdjustmentController) * reductionAlpha * (1 - sm) );

						//o.Albedo =  _Color;
						//o.Alpha = (1 - sm) * reductionAlpha;
					}

					if(d4 > 0 && d2 < 0){

						//This method takes a littlw while till the fod completely envelops the cone
						/*float sm = smoothstep(0, abs(dot(_LeftVector, _LeftFogVector)),
												 abs(dot(normalize(pixelRelToLight) ,_LeftFogVector)));*/

						//This method creates a uniform gradient effect
						float sm = smoothstep(abs (dot(_ActualLeftVector, _ActualLeftFogVector)), 1, 
													abs(dot(normalize(pixelRelToLight) , _ActualLeftFogVector)));

						finalColor = ( c + (_Color) + _LightMixtureAdjustmentController) * reductionAlpha * (1 - sm);
						//o.Albedo = _Color;
						//o.Alpha = (1-sm) * reductionAlpha;
					}
						return finalColor;
						//return c * c + (_Color);
				}


			ENDCG
		}
		

		/*Blend One One
		Pass{
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment frag
			ENDCG
		}*/
		
		/*Pass{
			CGPROGRAM
			//#pragma surface surf Lambert alpha
			#pragma vertex vert
			#pragma fragment frag
			#include "UnityCG.cginc"

			float4 _Color;
			sampler2D _MainTex;


			struct vertInput{
				float4 pos : POSITION;
				float2 texCoord : TEXCOORD0;
			};

			struct vertOutput{
				float4 pos : SV_POSITION;
				float2 texCoord : TEXCOORD0;
			};

			vertOutput vert(vertInput i){
				vertOutput o;
				o.pos = mul(UNITY_MATRIX_MVP, i.pos);
				o.texCoord = i.texCoord;
				return o;
			}

			half4 frag(vertOutput o) : COLOR{
				half4 c;
				c.rgb = tex2D(_MainTex, o.texCoord).rgb;
				
				c.a = _Color.a;
				return _Color;
			}

			

			ENDCG

		}*/


	}
	
}
